...
namespace Lennon.Utility.Drawing
{
    /// <summary>
    /// 验证码生成类
    /// </summary>
    public class ValidateCoder
    {
        ...
        private string GetRandomNums(int length)
        {
            ...
            {
                Random rnd = new Random((i + (int)DateTime.Now.Ticks) * (int)DateTime.Now.Ticks);
                ints[i] = rnd.Next(0, 9);
            }
        ...
        private string GetRandomNumsAndLetters(int length)
        {
            ...
            {
                Random rnd = new Random((result.Count + (int)DateTime.Now.Ticks) * (int)DateTime.Now.Ticks);
        ...
        /// <returns></returns>
        public string GetRandomHanzis(int length)
        {
            //汉字编码的组成元素，十六进制数
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                rnd = new Random(index1 * unchecked((int)DateTime.Now.Ticks) + i);
                int index2 = index1 == 13 ? rnd.Next(0, 7) : rnd.Next(0, 16);
                string str2 = baseStrs[index2];

                rnd = new Random(index2 * unchecked((int)DateTime.Now.Ticks) + i);
                int index3 = rnd.Next(10, 16);
                string str3 = baseStrs[index3];

                rnd = new Random(index3 * unchecked((int)DateTime.Now.Ticks) + i);
