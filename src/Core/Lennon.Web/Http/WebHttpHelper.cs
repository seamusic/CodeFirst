...
using Lennon.Web.Http.Security;


namespace Lennon.Web.Http
{
    /// <summary>
    /// WebAPI辅助操作类
    /// </summary>
    public static class WebHttpHelper
    {
        ...
        /// 获取HttpClient对象
        /// </summary>
        public static HttpClient GetHttpClient(string publicKey = null, string baseUrlFormat = "http://{0}:{1}/api/")
        {
            bool cryptoEnabled = ConfigurationManager.AppSettings["Communication-CryptoEnabled"].CastTo(false);
            if (cryptoEnabled)
            {
                publicKey.CheckNotNullOrEmpty("publicKey");
            }
            string host = ConfigurationManager.AppSettings["host"] ?? "localhost";
            string port = ConfigurationManager.AppSettings["port"] ?? "3000";
            string baseUrl = string.Format(baseUrlFormat, host, port);
            string hashType = ConfigurationManager.AppSettings["Communication-HashType"];
            var httpClient = cryptoEnabled
                ? HttpClientFactory.Create(new ClientCryptoHandler(publicKey, hashType))
                : new HttpClient();
