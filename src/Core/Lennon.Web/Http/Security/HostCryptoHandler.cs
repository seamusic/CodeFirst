...
using Lennon.Utility.Exceptions;
using Lennon.Utility.Secutiry;
using Lennon.Web.Http.Internal;
using Lennon.Web.Properties;


namespace Lennon.Web.Http.Security
{
    /// <summary>
    /// 服务端通信加密解密消息处理器
    /// </summary>
    public class HostCryptoHandler : DelegatingHandler
    {
        ...
        {
            if (!request.Headers.Contains(HttpHeaderNames.GmfClientPublicKey))
            {
                return CreateResponseTask(request, HttpStatusCode.BadRequest, "在请求头中客户端公钥信息无法找到。");
            }
            _publicKey = request.Headers.GetValues(HttpHeaderNames.GmfClientPublicKey).First();
        ...
        private HttpResponseMessage EncryptResponse(HttpResponseMessage response)
        {
            ...
            catch (Exception)
            {
                ...
                response.ReasonPhrase = Resources.Http_Security_Host_EncryptResponse_Failt;
                return response;
            }
        }
