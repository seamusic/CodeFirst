// -----------------------------------------------------------------------
//  <copyright file="IoCBundleActivator.cs" company="OSharp开源团队">
...
namespace Lennon.Plugin.Mvc
{
    ...
    public abstract class IoCBundleActivator : IBundleActivator
    {
        ...
        /// <summary>
        /// 获取 IoC插件最终整合并返回的Controller实例创建者
        /// </summary>
        public static IServiceResolver ControllerResolver { get; private set; }
        ...
        /// <summary>
        /// 重写以实现注册程序集中的<see cref="Lennon.Core.IDependency"/>接口的实现类型及其他映射类型
        /// </summary>
        /// <param name="assemblies">程序集</param>
        ...
        private void context_BundleStateChanged(object sender, BundleStateChangedEventArgs e)
        {
            ...
            else if (e.CurrentState == BundleState.Stopping)
            {
                ...
                if (_registerHostory.TryGetValue(e.Bundle.BundleID, out assemblies))
                {
                    ...
                    ControllerResolver = newResolver;
                }
            }
        }
